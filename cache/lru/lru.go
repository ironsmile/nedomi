...
// RemoveObjectsForKey removes all objects begining with the provided path and with the given cache key
func (tc *TieredLRUCache) RemoveObjectsForKey(key, path string) bool {
	tc.mutex.Lock()
	defer tc.mutex.Unlock()
	tc.logger.Logf("[%p] removing everything starting with %s and cache key %s from the cache", tc, path, key)
	defer tc.logger.Logf("[%p] finished removing everything starting with %s and cache key %s from the cache", tc, path, key)
	return tc.removeForPrefix(cacheKeyAndPathToPath(key, path))
}
