...
type CacheAlgorithm interface {
	...
	// RemoveObjectsForKey  removes all objects whose paths start with the
	// provided path and have cache key equal to the provided.
	// true is returned if any object index was remvoed
	RemoveObjectsForKey(key, path string) bool
}
